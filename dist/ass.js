    ...
    slices.forEach(function (slice) {
      ...
      slice.fragments.forEach(function (fragment) {
        ...
        if (!drawing) {
          ...
          cssText += "font-size:" + (this$1.scale *
            getRealFontSize(tag.fn, tag.fs)) + "px;";
          cssText += "color:" + (color2rgba(tag.a1 + tag.c1)) + ";";
          var scale = /Yes/i.test(this$1.info.ScaledBorderAndShadow)
            ? this$1.scale
            : 1;
          if (borderStyle === 1) {
        ...
        var hasTransfrom = transformTags.some(function (x) { return /^fsc[xy]$/.test(x) ? tag[x] !== 100 : !!tag[x]; }
        );
        if (hasTransfrom) {
          cssText += (vendor.transform) + "transform:" + (createTransform(tag)) + ";";
          if (!drawing) {
            cssText +=
              "transform-style:preserve-3d;word-break:normal;white-space:nowrap;";
          }
        }
        if (animationName) {
          cssText += createAnimation(
            animationName,
            end - start,
            Math.min(0, start - vct)
          );
        }
        if (drawing && tag.pbo) {
        ...
        encodeText(text, tag.q)
          .split("<br>")
          .forEach(function (html, idx) {
            ...
            if (drawing) {
              ...
              $span.innerHTML = html.replace(replaceSpaces, " ");
            }
